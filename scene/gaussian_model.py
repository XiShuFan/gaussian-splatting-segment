...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, cam_infos : int, spatial_lr_scale : float, full_opacity : bool = False, half_scale : bool = False):
        ...
        if full_opacity:
            opacities = self.inverse_opacity_activation(0.8 * torch.ones((fused_point_cloud.shape[0], 1), dtype=torch.float, device="cuda"))
        else:
            # TODO 初始透明度为0.1
    ...
    def load_ply(self, path, cam_infos, use_train_test_exp = False):
    ...
    def add_densification_stats(self, viewspace_point_tensor, update_filter, per_view_densify_weight):
        self.xyz_gradient_accum[update_filter] += torch.norm(viewspace_point_tensor.grad[update_filter,:2], dim=-1, keepdim=True) * per_view_densify_weight
